...
const columns: Column[] = [
    ...
    { key: 'dateCreated', label: 'Date Created', minWidth: 'min-w-[150px]' },
];
...
export default function InquiriessPage() {
    const [data, setData] = useState<Users[]>(staticUsers);
    const [searchTerm, setSearchTerm] = useState('');
    const [sortColumn, setSortColumn] = useState<keyof Users | null>(null);
    ...
    const [activeTab, setActiveTab] = useState<'faculty'>('faculty');
    ...
    const getUserCount = (key: string) => data.length;
    ...
    const renderCellContent = (item: Users, columnKey: keyof Users) => {
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex">
                            {tabs.map((tab) => (
                                <button
                                    key={tab.key}
                                    onClick={() => setActiveTab(tab.key as 'faculty')}
                                    className={`cursor-pointer relative flex items-center gap-2 px-6 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key
                                        ? 'border-[#C52233] text-[#C52233] bg-red-50'
                                        : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                        }`}
                                >
                                    <span>{tab.label}</span>
                                    <span className="inline-flex items-center justify-center w-5 h-5 text-xs font-semibold text-white bg-[#C52233] rounded-full">
                                        {getUserCount(tab.key)}
                                    </span>
                                </button>
                            ))}
                        </div>
